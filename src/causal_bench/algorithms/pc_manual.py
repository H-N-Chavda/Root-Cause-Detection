...
# Phase 3 wrapper
#
# Everything above this line is the Phase 1 implementation, unchanged: no logic
# edited, no public function renamed. Only the module's *file name* moved, from
# `pc.py` to `pc_manual.py`, so that `pc.py` could become the tigramite wrapper
# the Phase 3 layout calls for.
#
# This class exists as a correctness oracle, not as a benchmark entry. The test
# suite checks that it and tigramite's PC recover the same skeleton at the same
# alpha; their orientations legitimately differ, because this implementation
# uses the plain collider rule and tigramite defaults to the majority rule.
# ---------------------------------------------------------------------------

from typing import Any as _Any  # noqa: E402

from ..graph.representation import CausalGraph as _CausalGraph  # noqa: E402
from .base import CausalDiscoveryAlgorithm as _Base  # noqa: E402


def pc_result_to_graph(
    result: "PCResult", var_names: Sequence[str]
) -> _CausalGraph:
    """Converts this module's `PCResult` into the shared graph object.

    Convention mapping, stated explicitly because getting it backwards
    transposes the graph and every SHD then looks plausible but is wrong:

    * `PCResult.directed_edges` holds `(parent, child)` index pairs, i.e.
      *cause first*. The shared object wants `links[i, j, 0] == "-->"` to mean
      i causes j, so `(a, b)` maps to `links[a, b, 0] = "-->"` with no
      transpose.
    * `undirected_edges` and `conflicting_edges` are unordered `(i, j)` pairs
      with i < j. Undirected becomes `o-o`; conflicting becomes `x-x`, which is
      the mark tigramite uses for the same situation.
    """
    var_names = list(var_names)
    graph = _CausalGraph.empty(var_names, tau_max=0)
    for a, b in result.directed_edges:
        graph.links[a, b, 0] = "-->"
        graph.links[b, a, 0] = "<--"
    for i, j in result.undirected_edges:
        graph.links[i, j, 0] = graph.links[j, i, 0] = "o-o"
    for i, j in result.conflicting_edges:
        graph.links[i, j, 0] = graph.links[j, i, 0] = "x-x"
    return graph


class PCManualAlgorithm(_Base):
    """The repository's own PC implementation, behind the common interface."""

    name = "pc_manual"
    library = "causal-bench"
    handles_lags = False
    ...
    def check_assumptions(self, findings: Optional[Dict[str, _Any]]) -> List[str]:
        # Same Fisher-z test and same i.i.d. assumption as tigramite's PC, so
        # the same violations apply. Delegated rather than duplicated.
        from .pc import PCAlgorithm

        return PCAlgorithm().check_assumptions(findings)

    def _fit(
        self,
        data: np.ndarray,
        var_names: List[str],
        prior_knowledge: _Any = None,
    ) -> _CausalGraph:
        if prior_knowledge is not None:
            raise ValueError(
                "pc_manual takes no prior knowledge; the Phase 1 implementation "
                "has no mechanism for it and adding one would mean editing "
                "frozen code"
            )
        result = pc_algorithm(
            data,
            alpha=self.params["pc_alpha"],
            max_cond_set_size=self.params["max_cond_set_size"],
            variable_names=var_names,
        )
        graph = pc_result_to_graph(result, var_names)
        graph.meta = {
            "collider_rule": "plain (unshielded colliders, no majority vote)",
            "skeleton": "PC-stable (order independent)",
            "max_level_reached": result.max_level_reached,
            "has_cycle": bool(result.has_cycle),
            "n_conflicting_edges": len(result.conflicting_edges),
        }
        return graph
